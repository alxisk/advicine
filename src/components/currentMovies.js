...
class CurrentMovies extends Component {
  ...
  render() {
    ...
    if (!currentMovies.hasOwnProperty('upcomingMovies') || currentMovies.isFetching) {
      return (
        ...
        <div className="wrap">
          <div className="current-movies__block">
            ...
            <ul className="current-movies__list">
              {upcomingMovies
                .slice(0, 8)
                .map(item => <CurrentMoviesItem key={item.id} {...item} />)}
            </ul>
          </div>
          <div className="current-movies__block">
            <h2 className="current-movies__block-name">Latest TV Shows</h2>
            <ul className="current-movies__list">
              {latestTVShows.slice(0, 8).map(item => <CurrentMoviesItem key={item.id} {...item} />)}
            </ul>
          </div>
        </div>
      </section>
