...
export const fetchCurrentMovies = () => dispatch => {
  ...
  return Promise.all([latestTVShows, upcomingMovies]).then(responses =>
    dispatch(recieveCurrentMovies(responses[0], responses[1]))
  )
}
...
export const fetchBestMovies = (page, scrollPosition) => dispatch => {
  ...
  return tmdbApi
    .getBestMovies(page)
    .then(response => dispatch(recieveBestMovies(response, scrollPosition)))
}
