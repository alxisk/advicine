const tmdbApi = {
  ...
  getAdvSearchResults(date, genres, lang, runtime) {
    ...
    return fetch(
      `${this.baseUrl}discover/movie?${
        this.key
      }&language=en-US&${dateStr}&${runtimeStr}&${genreStr}&with_original_language=${lang}`
    )
      .then(response => response.json(), error => console.error(error))
      .then(json => json.results)
  },
  ...
  getDetails(titleId, tv) {
    if (tv) {
      return fetch(`${this.baseUrl}tv/${titleId}?${this.key}&language=en-US`).then(
        response => response.json(),
        error => console.error(error)
      )
    } else {
      return fetch(`${this.baseUrl}movie/${titleId}?${this.key}&language=en-US`).then(
        response => response.json(),
        error => console.error(error)
      )
    }
